...
> 状态：**待实现** · 日期：2026-06-25  
> 决策：用户选定 **方案一（ChatCubit 全栈）** — 走真实 `openSessionTab` → `SessionLaunchService` → PTY spawn → mock Anthropic API → 两 Claude 成员经 teammate-bus MCP 互发消息。
...
mixed_team_claude_bus_integration_test.dart
  │
  ├─ MockAnthropicServer (loopback, route by API key)
  ├─ setUpTestAppStorage + test providers + SessionRepository
...
**文件：** `client/test/integration/support/mock_anthropic_server.dart`
...
| 文件 | 职责 |
|------|------|
| `test/integration/mixed_team_claude_bus_integration_test.dart` | 主集成测 |
| `test/integration/support/mock_anthropic_server.dart` | Mock API + 剧本 |
| `test/integration/support/mixed_team_claude_harness.dart` | 可选：providers 写入、poll helpers、stdin kickoff |
| `test/integration/support/mock_anthropic_server_test.dart` | Mock SSE 格式单测（非 integration tag） |
| `docs/DEVELOPMENT.md` | 运行命令段落 |
...
- ChatCubit + Docker SSH 远程成员（复用 `docker_ssh_server.dart`）
- macOS CI nightly job
- 3+ 成员 / task queue 场景
