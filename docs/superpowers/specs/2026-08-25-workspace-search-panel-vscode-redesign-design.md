...
┌─────────────────────────────────────┐
│ [⌄] ┌──────────────────────────┐    │
│     │ codexPluginManifestPaths │Aa .* │  ← 搜索行：chevron 切换替换行；
│     └──────────────────────────┘    │    字段内嵌 Aa/..* 切换
│     ┌──────────────────────────┐    │
│     │ 替换                     │    ⤿│  ← 替换行（默认收起）：
│     └──────────────────────────┘    │    右侧 全部替换 图标（带确认框）
...
- `⋯` 图标在存在非默认过滤条件时（include/exclude 非空 或 gitignore 被关闭）高亮提示（VSCode 同款行为）。
- 所有字段全宽，切换图标内嵌字段右侧（替代 `Wrap(FilterChip)`）。搜索字段仅 Aa（大小写）+ .*（正则）两个切换——`TpSearchOptions` 无全字匹配（ab）与保留大小写（AB）字段，且本 spec 非目标禁止改引擎，故不做这两个切换。
...
| `widgets/right_tools/search_panel_form.dart`（新增） | chevron 切换、`⋯` 切换、详情区（包含/排除/gitignore）；字段直接复用 `FindField`（`width` 省略即全宽），不新建字段组件 |
| `widgets/right_tools/search_panel_results.dart`（改造） | 统计行、可折叠文件组、hover 替换图标、行匹配渲染 |

**复用**：`FindBarPalette`、`FindField`（全宽）、`FindToggleButton`、`FindActionButton`、现有 SVG 资产。
