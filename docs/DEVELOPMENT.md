...
Pure Dart `teampilot_*` packages under `client/packages/` (`teampilot_fs`, `teampilot_apply`, `teampilot_scheduler`): run `dart analyze` and `dart test` in that package directory — not `flutter test` or `tool/run_tests.dart`.
