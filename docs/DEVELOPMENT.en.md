...
**Release (recommended):** Bump `version:` in `client/pubspec.yaml` before merging to `main`. [Auto Tag on Version Bump](../.github/workflows/auto-tag.yml) detects the change, pushes a **`v*`** tag, and triggers [Release Packages](../.github/workflows/release.yml) to build all platforms and publish a **GitHub Release**. Release notes are still generated by [git-cliff](https://git-cliff.org/) from **Conventional Commits since the previous tag**—same as when you tag manually.

You can still run `git tag vX.Y.Z && git push origin vX.Y.Z`, or use **workflow_dispatch** on Release Packages with any `ref` (no GitHub Release unless that ref is already a tag).
