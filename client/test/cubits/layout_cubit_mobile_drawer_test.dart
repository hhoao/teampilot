...
void main() {
  ...
  test(
    'setMobileDrawerMode tools flips intents without losing mode memory on close',
    () async {
      final cubit = LayoutCubit();
      addTearDown(cubit.close);
      cubit.openMobileWorkspaceDrawer();
      await cubit.setMobileDrawerMode(
  ...
  test(
    'setMobileDrawerMode never emits both sides false while switching',
    () async {
      final cubit = LayoutCubit();
      addTearDown(cubit.close);
      cubit.openMobileWorkspaceDrawer();
  ...
  test(
    'setMobileDrawerMode tools with composeLanding sets landing override',
    () async {
      final cubit = LayoutCubit();
      addTearDown(cubit.close);
  ...
  test(
    'legacy setSidebarVisible(true) syncs mobileDrawerMode to chat',
    () async {
      final cubit = LayoutCubit();
      addTearDown(cubit.close);
  ...
  test(
    'legacy setRightToolsVisible(true) syncs mobileDrawerMode to tools',
    () async {
      final cubit = LayoutCubit();
      addTearDown(cubit.close);
