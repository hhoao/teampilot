...
void main() {
  ...
  test(
    'discardMemberTerminal removes the shell and marks it reclaimed',
    () async {
      ...
      expect(
        tab.memberShells.containsKey(memberId),
        isFalse,
      ...
      expect(
        shell.isRunning,
        isFalse,
        reason: 'shell transport is disconnected',
      );
      expect(
        podNotifications,
        greaterThan(0),
  ...
  test(
    'discardMemberTerminal is a no-op when the shell is not running',
    () async {
      final team = _withBuiltinMembers(
        TeamProfile(
          id: LaunchProfileProvisioner.defaultNativeTeamId,
      ...
      final idleShell = FakeTerminalSession(
        executable: 'bin',
        fs: InMemoryFilesystem(),
      );
