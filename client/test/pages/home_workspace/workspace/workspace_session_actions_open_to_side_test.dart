...
/// Boots a tmp workspace with two persisted sessions, a ChatCubit wired to a
/// real WorkbenchCubit through the bridge, and session A already open
...
setupWorkspace({bool aConnectImmediately = true}) async {
  ...
  await cubit.requestOpenSession(
    SessionOpenRequest(
      ...
      workspace: workspace,
      repo: repo,
      connectImmediately: aConnectImmediately,
    ),
  );
...
void main() {
  ...
  testWidgets(
    'openWorkspaceSessionTabToSide keeps a focused preview tab in place',
    (tester) async {
      SharedPreferences.setMockInitialValues({});
      final prefs = await SharedPreferences.getInstance();
      final sessionPreferencesCubit = SessionPreferencesCubit(
      ...
      final result = await tester.runAsync(
        () => setupWorkspace(aConnectImmediately: false),
      );
      expect(result, isNotNull);
      final (chatCubit, workbench, workspace, sessionA, sessionB, repo) =
          result!;
      ...
      // Session A stays in the focused (left) group — the side open must not
      // replace it through the strip's preview slot — and B lands in a new
      // right group.
      final layout = workbench.centerLayout(workspace.workspaceId);
      expect(layout.leafGroupIds, hasLength(2));
      expect(
