...
void main() {
  ...
  Future<void> writeMinimalRoster({
    ...
    final members = [
      for (final name in names)
        {'name': name, 'agentType': agentTypes[name] ?? name},
    ];
    await File(
      p.join(rosterDir, 'config.json'),
    ).writeAsString(jsonEncode({'name': cliTeamName, 'members': members}));
  ...
  test(
    'expectClaudeInboxAbsent for type id when only pod inbox exists',
    () async {
  ...
  test(
    'ClaudeInboxUnreadWatch fails when the inbox never receives a message',
    () async {
      await writeMinimalRoster(inboxMemberIds: const ['developer-0']);
      final watch = ClaudeInboxUnreadWatch(
        claudeDir: claudeDir,
