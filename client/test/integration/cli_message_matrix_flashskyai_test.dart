...
void main() {
  ...
  test(
    'flashskyai simple: History compose → user bubble → ≥3 assistant bubbles',
    () async {
      IntegrationPrerequisites.skipUnlessNativePty();
      final flashskyaiPath = IntegrationPrerequisites.requireFlashskyaiPath();
      if (flashskyaiPath == null) return;

      final harness = CliMessageMatrixHarness.forCli(
        CliTool.flashskyai,
        mode: CliMatrixMode.simple,
        cliPath: flashskyaiPath,
      );
      ...
      addTearDown(() async {
        ...
        // Let flashskyai PTY children release config dir handles before tearDown.
      ...
      try {
        ...
        // Settle back to the composer between submits so flashskyai does not
        // queue.
        const prompts = [
  ...
  test(
    'flashskyai mixed: History compose → collab ≥3 assistant bubbles + bus',
    () async {
      IntegrationPrerequisites.skipUnlessNativePty();
      final flashskyaiPath = IntegrationPrerequisites.requireFlashskyaiPath();
      if (flashskyaiPath == null) return;
  ...
  test(
    'flashskyai native: History compose → collab ≥3 assistant bubbles',
    () async {
      IntegrationPrerequisites.skipUnlessNativePty();
      final flashskyaiPath = IntegrationPrerequisites.requireFlashskyaiPath();
      if (flashskyaiPath == null) return;
