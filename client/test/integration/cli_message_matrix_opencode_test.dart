...
void main() {
  ...
  test(
    'opencode simple: History compose → user bubble → ≥3 assistant bubbles',
    () async {
      IntegrationPrerequisites.skipUnlessNativePty();
      final opencodePath = IntegrationPrerequisites.requireOpencodePath();
      if (opencodePath == null) return;
      ...
      addTearDown(() async {
        ...
        // Let OpenCode PTY children release OPENCODE_CONFIG_DIR handles.
      ...
      try {
        ...
        // Settle back to the composer between submits so OpenCode does not
        // queue.
        const prompts = [
  ...
  test(
    'opencode mixed: History compose → collab ≥3 assistant bubbles + bus',
    () async {
      IntegrationPrerequisites.skipUnlessNativePty();
      final opencodePath = IntegrationPrerequisites.requireOpencodePath();
      if (opencodePath == null) return;
