...
void main() {
  TerminalDropIngestor ingestor(
    ...
    target: target,
    behavior: behavior,
    crossNamespaceStrategy: strategy ?? const RejectCrossNamespaceStrategy(),
  );
  ...
  test(
    'bracketedNoSubmit pastes without submitting for full-screen TUIs',
    () async {
      final sink = _FakeSink();
      await ingestor(
        sink,
        target: const RuntimeTarget.localPosix(),
        behavior: TerminalPathDropBehavior.defaultFor(
          usesFullScreenInput: true,
        ),
      ).consume(_payload(['/repo/x.dart'], const PathNamespace.localPosix()));
  ...
  test(
    'cross-namespace paths are rejected and nothing is written by default',
    () async {
      final sink = _FakeSink();
      final outcome = await ingestor(
        sink,
  ...
  test(
    'a cross-namespace strategy can resolve a deliverable remote path',
    () async {
      final sink = _FakeSink();
      final outcome = await ingestor(
        sink,
  ...
  test(
    'quoting follows the behavior PathQuoting (none leaves spaces raw)',
    () async {
      final sink = _FakeSink();
      await ingestor(
        sink,
        ...
        _payload(['/repo/my file.txt'], const PathNamespace.localPosix()),
      );
