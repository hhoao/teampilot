...
void main() {
  group('GitWorktreeService.parseWorktreeList', () {
    test('parses NUL-delimited porcelain blocks; first is main', () {
      ...
      final list = GitWorktreeService.parseWorktreeList(
        out,
        nulDelimited: true,
      );
    ...
    test('parses detached and bare entries', () {
      const out =
          'worktree /repo\x00HEAD abc\x00bare\x00\x00'
    ...
    test('preserves a path containing a space (NUL split, not space split)', () {
      const out =
          'worktree /my repo/main\x00HEAD abc\x00branch refs/heads/main\x00\x00';
      final list = GitWorktreeService.parseWorktreeList(
        out,
        nulDelimited: true,
      );
