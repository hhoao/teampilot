...
ProcessResult _badRef([
  String detail = 'fatal: bad object refs/remotes/origin/xxx',
]) => ProcessResult(0, 128, '', '$detail\n');

/// 一条可解析的 commit 行：`*` + \x1e + 七字段。
const String _oneRow =
    '*\x1eh1\x1f\x1fAnn\x1fann@x\x1f1700000000\x1f\x1fsubj\n';
...
void main() {
  ...
  test(
    'graphRows hash mode adds no server-side filter (client filters)',
    () async {
      final fake = _FakeRunner({'log': ok('')});
      final svc = GitHistoryService(
        runner: LocalGitCommandRunner(runner: fake.call),
  ...
  test(
    'graphRows falls back to --branches --tags when --all hits bad ref',
    () async {
  ...
  test(
    'graphRows throws last GitException after exhausting fallbacks',
    () async {
      final fake = _FakeRunner({
        'log --all': _badRef('fatal: bad object refs/remotes/origin/a'),
        'log --branches --tags': _badRef('fatal: bad object refs/tags/t'),
  ...
  test('commitDetail parses show + diff-tree name-status', () async {
    ...
    final fake = _FakeRunner({
      'show': ok(
        '$sep h1${f}h0${f}Ann${f}ann@x${f}1700000000${f}subj'
        '${f}full body\n',
      ),
  ...
  test('stashList parses selectors; empty output yields empty list', () async {
    ...
    final svc2 = GitHistoryService(
      runner: LocalGitCommandRunner(runner: emptyFake.call),
    );
