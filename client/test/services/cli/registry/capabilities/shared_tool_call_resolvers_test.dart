...
void main() {
  ...
  group('共享层治理：key/toolName 集精确固定', () {
    ...
    test(
      'edit 各集只含共享项（strreplace/editnotebook 下沉至 cursor，'
      'oldString/newString 下沉至 opencode，file/target_file/start_line 无证据移除）',
      () {
        expect(SharedToolCallResolverKeys.editToolNames, {
          'edit',
          'notebookedit',
        });
        ...
        expect(SharedToolCallResolverKeys.editNewStringKeys, [
          'new_string',
          'new_source',
        ]);
      },
    ...
    test('diff 集保持共享（codec 能力层：codex + opencode 共用配置族）', () {
      ...
      expect(SharedToolCallResolverKeys.diffPatchKeys, [
        'patch',
        'diff',
        'input',
      ]);
    ...
    test('shell 各集只含共享项（shell/run_shell_command/run_terminal_cmd '
      ...
      expect(SharedToolCallResolverKeys.shellToolNames, {
        'bash',
        'shell_command',
        'exec_command',
      });
  ...
  group('共享层基线族仍可解析（claude/flashskyai 同源面）', () {
    ...
    test('Edit{file_path, old_string, new_string} 解析出 hunk', () {
      final target = shared.editResolver.resolve(
        part(
          'Edit',
          args: {
            'file_path': 'a.txt',
    ...
    test('Write{file_path, content} 解析出 hunk', () {
      final target = shared.editResolver.resolve(
        part('Write', args: {'file_path': 'new.txt', 'content': 'line1'}),
      );
      expect(target, isNotNull);
    ...
    test('Read{file_path} 解析出文件目标', () {
      final target = shared.fileResolver.resolve(
        part('Read', args: {'file_path': 'a.dart'}),
      );
      expect(target, isNotNull);
    ...
    test('Bash{command} 解析出命令', () {
      final target = shared.shellResolver.resolve(
        part('Bash', args: {'command': 'ls'}),
      );
      expect(target, isNotNull);
...
*** End Patch''',
  ...
  group('下沉项不在共享层解析（由拥有者 CLI 提供）', () {
    ...
    test('Shell（cursor 特有名）不在共享 shellResolver 解析', () {
      expect(
        shared.shellResolver.resolve(part('Shell', args: {'command': 'pwd'})),
        isNull,
      );
    });
    ...
    test('无证据别名不在共享层解析（writefile/create_file/readfile/'
      ...
      expect(
        shared.editResolver.resolve(
          part('writefile', args: {'file_path': 'a', 'content': 'b'}),
        ),
        isNull,
      );
      expect(
        shared.editResolver.resolve(
          part('create_file', args: {'file_path': 'a', 'content': 'b'}),
        ),
        isNull,
      );
      expect(
        shared.fileResolver.resolve(part('readfile', args: {'path': 'a'})),
        isNull,
      );
      expect(
        shared.shellResolver.resolve(
          part('run_shell_command', args: {'command': 'pwd'}),
        ),
        isNull,
      );
      expect(
        shared.shellResolver.resolve(
          part('run_terminal_cmd', args: {'command': 'pwd'}),
        ),
        isNull,
      );
    });
    ...
    test('cursor 特有键 path/contents 不在共享层解析（G-4 下沉项）', () {
      ...
      expect(
        shared.editResolver.resolve(
          part('Write', args: {'file_path': 'a.txt', 'contents': 'b'}),
        ),
        isNull,
  ...
  group('下沉项在拥有者 CLI 可解析', () {
    ...
    test('cursor：StrReplace / EditNotebook 解析出 hunk', () {
      final strTarget = cursor.editResolver.resolve(
        part(
          'StrReplace',
          args: {
            'file_path': 'a.txt',
      ...
      final notebookTarget = cursor.editResolver.resolve(
        part(
          'EditNotebook',
          args: {'notebook_path': '/tmp/demo.ipynb', 'new_source': 'print(1)'},
        ),
    ...
    test('opencode：edit{filePath, oldString, newString} 解析出 hunk', () {
      final target = opencode.editResolver.resolve(
        part(
          'edit',
          args: {'filePath': 'a.txt', 'oldString': 'x', 'newString': 'y'},
        ),
