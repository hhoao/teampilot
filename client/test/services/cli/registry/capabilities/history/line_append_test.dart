...
void main() {
  ...
  for (final (cli, fixture) in [
    ...
    test(
      '$cli: lineAppend replays fixture identically to full parse',
      () async {
        final capability = registry.capability<AiHistoryCapability>(cli)!;
        ...
        expect(
          ...
          reason:
              'incremental lineAppend must produce the same message id '
    ...
    test('$cli: consumed flags match event semantics', () async {
      ...
      expect(
        ...
        reason:
            'events producing/mutating messages must return true, '
  ...
  test(
    ...
    () async {
      final capability = registry.capability<AiHistoryCapability>(
        CliTool.claude,
      )!;
      final messages = <AiMessage>[];
      var seq = 0;
      expect(
        capability.lineAppend!(messages, {
          'type': 'assistant',
          'message': {
            'role': 'assistant',
            'content': [
  ...
  test(
    'claude: discarded id-less events do not consume fallback ids',
    () async {
      final capability = registry.capability<AiHistoryCapability>(
        CliTool.claude,
      )!;
  ...
  test(
    'cursor: discarded id-less events do not consume fallback ids',
    () async {
      final capability = registry.capability<AiHistoryCapability>(
        CliTool.cursor,
      )!;
