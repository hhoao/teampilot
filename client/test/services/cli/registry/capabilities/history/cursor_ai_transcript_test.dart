...
void main() {
  ...
  test(
    ...
    () async {
      ...
      expect(asst.parts.whereType<AiTextPart>().single.text, 'hi from cursor');
...
''';
      ...
      expect(
        messages.single.parts.whereType<AiToolCallPart>().map(
          (t) => t.toolName,
        ),
        ['Grep', 'Read', 'Shell'],
      );
  ...
  test('unwraps Cursor <user_query> and strips <timestamp>', () async {
    const raw = '''
...
''';
  ...
  test(
    ...
    () async {
      ...
      expect(asst.parts.whereType<AiTextPart>().single.text, '你好。需要我帮你做什么？');
    },
  );

  test('message id priority is uuid -> event id -> message.id -> lazy '
      'fallback cursor-{seq} (G1)', () async {
...
''';
  ...
  test('thinking blocks produce no reasoning part; [REDACTED] never surfaces '
      '(G3)', () async {
...
''';
    ...
    expect(
      messages.whereType<AiMessage>().expand((m) => m.parts),
      isNot(contains(isA<AiReasoningPart>())),
    );
    expect(messages, hasLength(1));
    expect((messages.single.parts.single as AiTextPart).text, 'visible answer');
  });

  test('id-less tool_use gets non-empty synthetic {messageId}-tool-{seq} '
      'toolCallId (G4)', () async {
...
''';
  ...
  test(
    'non-Map tool input yields args null, never a bare string (G2)',
    () async {
      const raw = '''
...
''';
  ...
  test('ApplyPatch 字符串 input 保留到 argsText（FREEFORM，Task 6 G-4 重校）', () async {
...
''';
  ...
  test('fixture: chat-strreplace-write（真实键形 StrReplace/Write/ApplyPatch）'
      '经 adapter + resolver 全链路解析出 hunk', () async {
    ...
    final tools = messages
        ...
        .toList();
    expect(tools.map((t) => t.toolName), ['StrReplace', 'Write', 'ApplyPatch']);
  ...
  test('locateCursorTranscript returns agent-transcripts jsonl', () async {
    ...
    final bundle = await locateCursorTranscript(
      ctx(configDir: base.path, persistedNativeId: 'chat-aaaa-bbbb-cccc-dddd'),
    );
  ...
  test(
    'locateCursorTranscript cacheToken changes when terminal file added',
    () async {
      await copyFixtureTree();
      final context = ctx(
        configDir: base.path,
