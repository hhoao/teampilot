...
void main() {
  ...
  test(
    'ClaudeAiTranscriptAdapter parses user text and correlates tool_result',
    () async {
      ...
      final messages = await adapter.parse(
        AiTranscriptBundle(
          ...
          fragments: [AiTranscriptFragment(name: 'basic.jsonl', bytes: bytes)],
        ),
      );
...
''');
  ...
  test(
    ...
    () {
      ...
      expect(
        appendClaudeJsonlEvent(messages, {
          'type': 'assistant',
          'message': {
            'role': 'assistant',
            'content': [
      ...
      expect(
        fallbackSeq,
        0,
        reason: 'message.id present must not consume fallback',
      );
      ...
      expect(
        fallbackSeq,
        1,
        reason: 'id-less consumed event takes one fallback id',
      );
  ...
  test('G3: encrypted/empty thinking yields no reasoning part', () {
    ...
    expect(
      parts.whereType<AiReasoningPart>(),
      isEmpty,
  ...
  test('G4: id-less tool_use is skipped, not synthesized', () {
    ...
    expect(
      appendClaudeJsonlEvent(withText, {
        'type': 'assistant',
        'message': {
          'role': 'assistant',
          'content': [
    ...
    expect(
      parts.whereType<AiToolCallPart>(),
      isEmpty,
  ...
  test(
    'appendClaudeJsonlEvent merges streamed assistant lines across calls',
    () {
      final messages = <AiMessage>[];
      expect(
        appendClaudeJsonlEvent(messages, {
