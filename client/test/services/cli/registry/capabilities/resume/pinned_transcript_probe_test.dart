...
void main() {
  group('probePinnedTranscript bucket scan', () {
    test('prefers a .jsonl transcript file across buckets over a session-dir '
        'in a sibling bucket', () async {
    ...
    test(
      'falls back to session-dir match when no .jsonl exists anywhere',
      () async {
        final fs = InMemoryFilesystem();
        const root = '/layout';
        const sid = 'task-2';
    ...
    test('matchDirectories:false ignores a session-dir-only bucket (transcript '
        'location semantics)', () async {
      final fs = InMemoryFilesystem();
      const root = '/layout';
      const sid = 'task-4';
    ...
    test('matchDirectories:false still finds the .jsonl file when both a '
        'sidecar dir and the real file exist', () async {
      final fs = InMemoryFilesystem();
      const root = '/layout';
      const sid = 'task-5';
    ...
    test('chooses the fuller .jsonl when the same session id exists in two '
        'buckets (empty residual vs real transcript)', () async {
      final fs = InMemoryFilesystem();
      const root = '/layout';
      const sid = 'task-7';
    ...
    test(
      'resume (matchDirectories:true) also prefers the fuller file',
      () async {
        final fs = InMemoryFilesystem();
        const root = '/layout';
        const sid = 'task-8';
    ...
    test(
      ...
      () async {
        ...
        await fs.ensureDir(
          fs.pathContext.join(root, 'projects', 'other-bucket', sid),
        );
        await fs.ensureDir(
          fs.pathContext.join(root, 'projects', 'main-bucket', sid),
        );
