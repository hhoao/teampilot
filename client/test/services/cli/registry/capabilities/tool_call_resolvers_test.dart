...
void main() {
  ...
  group('claude', () {
    ...
    test('Edit 工具解析出 hunk（file_path/old_string/new_string）', () {
      final target = claude.editResolver.resolve(
        part(
          'Edit',
          args: {
            'file_path': 'a.txt',
    ...
    test('Read 工具解析出文件目标（file_path/offset/limit → 行区间）', () {
      final target = claude.fileResolver.resolve(
        part('Read', args: {'file_path': 'a.dart', 'offset': 10, 'limit': 5}),
      );
      expect(target, isNotNull);
    ...
    test('Bash 工具解析出命令', () {
      final target = claude.shellResolver.resolve(
        part('Bash', args: {'command': 'ls -la'}),
      );
      expect(target, isNotNull);
    ...
    test(
      'EnterPlanMode 归类为 plan；TaskGet/TaskList/TaskOutput/TaskStop 归类为 task',
      () {
        expect(
  ...
  group('codex', () {
    ...
    test('apply_patch FREEFORM（argsText 直接是 patch 文本）解析出 hunk', () {
      final target = codex.editResolver.resolve(
        part(
          'apply_patch',
...
*** End Patch''',
  ...
  group('flashskyai', () {
    ...
    test('Edit 工具通过共享 codec 解析出 hunk（file_path/old_string/new_string）', () {
      final target = flashskyai.editResolver.resolve(
        part(
          'Edit',
          args: {
            'file_path': 'a.txt',
    ...
    test('Write 工具通过共享 codec 解析出 hunk（file_path/content）', () {
      final target = flashskyai.editResolver.resolve(
        part('Write', args: {'file_path': 'b.txt', 'content': 'line1\nline2'}),
      );
      expect(target, isNotNull);
    ...
    test('Read{file_path} 解析出文件目标（夹具实测 key）', () {
      final target = flashskyai.fileResolver.resolve(
        part('Read', args: {'file_path': '/tmp/demo/SKILL.md'}),
      );
      expect(target, isNotNull);
    ...
    test('Bash{command, description} 解析出命令（夹具实测 key）', () {
      final target = flashskyai.shellResolver.resolve(
        part(
          'Bash',
          args: {
            'command': 'pwd',
    ...
    test('G-2 接受差异固化：subagentToolNames 仅 agent/task（无 workflow run 侧解析）', () {
      expect(const FlashskyaiAiHistoryCapability().subagentToolNames, {
        'agent',
        'task',
      });
    });
