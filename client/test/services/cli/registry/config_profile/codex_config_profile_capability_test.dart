...
void main() {
  group('CodexTeamBusOverlay', () {
    ...
    test(
      'registers teammate-bus MCP and provisions a Stop hook script',
      () async {
        ...
        expect(
          toml,
          contains(
            'tool_timeout_sec = ${CodexTeamBusOverlay.busToolTimeoutSec}',
          ),
        );
        expect(
          toml,
  ...
  group('CodexTeamBusOverlay remote stop hook', () {
    ...
    test(
      'provisionStopHook includes X-Bus-Token for remote idle endpoints',
      () async {
        const idle = MemberBusIdleEndpoint(
          url: 'http://127.0.0.1:54321/idle',
          token: 'sess-tok',
