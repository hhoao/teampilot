...
void main() {
  test(
    'provisionForCli returns empty result when CLI has no PromptCapability',
    () async {
  ...
  test('provisionForCli materializes via the CLI capability', () async {
    ...
    final path =
        result.environment[MemberRoleProvision.appendSystemPromptFileEnvKey]!;
    expect(await fs.stat(path), isNotNull);
    expect(await fs.readString(path), contains('You are the reviewer.'));
  });

  test('provisionForCli returns assembly diagnostics from providers', () async {
    final base = await Directory.systemTemp.createTemp('prompt_hub_diag_');
    addTearDown(() async {
      if (await base.exists()) await base.delete(recursive: true);
    });
    final fs = LocalFilesystem();
    ...
    final result = await const PromptHubService().provisionForCli(
      ...
      providers: const [
        _HubProvider('first', 'source-first', 'one'),
        _HubProvider('second', 'source-second', 'two'),
      ],
    );

    expect(result.written, isTrue);
    expect(result.diagnostics, hasLength(1));
    expect(result.diagnostics.single.providerId, 'second');
    expect(result.diagnostics.single.message, contains('first'));
    expect(result.diagnostics.single.message, contains('source-first'));
  });
}

final class _HubProvider implements PromptContributionProvider {
  const _HubProvider(this.providerId, this.sourceId, this.content);
  ...
  Iterable<PromptContribution> provide(PromptProviderContext context) => [
    PromptContribution(
      ...
      title: 'Hub shared',
      content: content,
      mergeRole: PromptMergeRole.section,
