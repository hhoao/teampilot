...
void main() {
  test(
    'composeProcessExitFailureMessage keeps exit stub alone when no output',
    () {
      expect(
  ...
  test('composeProcessExitFailureMessage prepends recent PTY text', () {
    const output = 'Error: [unavailable] connect ETIMEDOUT 28.0.0.8:443';
    expect(
  ...
  test('composeProcessExitFailureMessage soft-caps extremely long output', () {
    ...
    expect(composed.length, lessThan(output.length + 80));
  });
}
