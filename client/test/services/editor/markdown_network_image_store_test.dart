...
void main() {
  ...
  test(
    'concurrency gate never exceeds maxConcurrent in-flight fetches',
    () async {
      var inFlight = 0;
      var peak = 0;
  ...
  test(
    'disk cache serves bytes without network on second store instance',
    () async {
      var fetches = 0;
      ...
      final first = MarkdownNetworkImageStore(cacheDir: cacheDir, httpGet: get);
  ...
  test('prefetch loads urls through the store', () async {
    ...
    expect(
      seen,
      containsAll(['https://example.com/1.png', 'https://example.com/2.png']),
    );
