...
void main() {
  test(
    'hasPendingDoorbell true only while doorbelled with unread mail',
    () async {
      ...
      await bus.readMessages('worker', unreadOnly: true, markRead: true);
  ...
  test(
    'markMemberDiscarded returns a running member to declared, inbox kept',
    () async {
      final bus = TeamBus(launcher: FakeMemberLauncher());
      final node = AgentNode.test(
        memberId: 'worker',
