...
import 'package:teampilot/services/chat/conversation/follow_up/follow_up_queue_drainer.dart';
...
void main() {
  ...
  test(
    'idle edge drains one head then waits for next idle after latch',
    () async {
      drainer = FollowUpQueueDrainer(
        store: store,
        deliver: (s, text) async {
  ...
  test(
    'idle edge during in-flight drains remaining after first completes',
    () async {
  ...
  test(
    'successful drain while still idle continues with remaining heads',
    () async {
      store.enqueue(seat, 'one');
      store.enqueue(seat, 'two');
