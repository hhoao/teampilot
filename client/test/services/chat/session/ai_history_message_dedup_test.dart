...
import 'package:teampilot/services/chat/conversation/history/ai_history_message_dedup.dart';
...
void main() {
  ...
  test(
    'same text, different ids, tool result asymmetry keeps the completed one',
    () {
  ...
  test('legitimate duplicate USER messages are kept', () {
    final a = _msg('u1', role: AiRole.user, texts: ['没问题']);
    final b = _msg('u2', role: AiRole.user, texts: ['没问题']);
    final result = dedupeAiHistoryMessages([a, b]);
  ...
  test(
    'tool-only assistant messages are never deduped (empty text signature)',
    () {
  ...
  test(
    'same text, equal results, no superset keeps both and reports the pair',
    () {
      final a = _msg(
        'asst-a',
        texts: ['prose'],
  ...
  test('3+ group records each undecided unordered pair once', () {
    ...
    expect(result.removed, isEmpty);
    expect(result.undecidedPairs.map((p) => '${p.$1.id},${p.$2.id}').toList(), [
      'asst-a,asst-b',
      'asst-a,asst-c',
      'asst-b,asst-c',
