...
void main() {
  ...
  test('reuseHistoryMessageIdentity keeps next when same id streamed', () {
    ...
    expect((reused.single.parts.single as AiTextPart).text, 'hello world');
  });

  test(
    'reuseHistoryMessageIdentity uses cheap equality for large tool results',
    () {
      final result =
          'HEAD'.padRight(64, 'H') + ('x' * 4000) + 'TAIL'.padLeft(64, 'T');
