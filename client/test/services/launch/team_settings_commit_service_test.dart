...
import 'package:teampilot/services/chat/launch/session/member_placement_save.dart';
import 'package:teampilot/services/chat/launch/session/team_settings_commit_service.dart';
...
void main() {
  ...
  test(
    'commit persists profile + placement and patches the chat snapshot',
    () async {
      ...
      final workspace = await repo.createWorkspace(const [
  ...
  test(
    'commit returns false and persists nothing when lead placement invalid',
    () async {
      final launch = LaunchProfileCubit(
        storage: testHomeStorage,
        repository: testLaunchProfileRepository(
