...
void main() {
  ...
  test('opencode seeds home shared plugin deps before reconcile', () async {
    final homeFs = InMemoryFilesystem();
    ...
    expect(
      (await workFs.stat('/remote/cli-defaults/opencode/package.json')).isFile,
      isTrue,
    );
  });

  test('non-opencode cli does not seed shared plugin deps', () async {
    final fs = seeded();
    var seedCalls = 0;
    final m = RemoteAppDataMaterializer(
      loadLocalCredentials: (_) async => const [],
