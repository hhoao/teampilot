...
void main() {
  test(
    'materializes ancestry to machineRoot and closes inheritance in-root',
    () async {
  ...
  test(
    'second reconcile with unchanged content skips re-copy (manifest hit)',
    () async {
      final homeFs = InMemoryFilesystem();
      await homeFs.writeString('/home/cli-defaults/claude/agents/x.md', 'A');
  ...
  test('changing one file re-copies only that file', () async {
    ...
    await homeFs.writeString(
      '/home/cli-defaults/claude/agents/x.md',
      'A-changed',
    );
    await m.reconcile(tools: {'claude'}, workspaceId: 'w1');
