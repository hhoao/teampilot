...
void main() {
  group('MarketplaceSharedStore', () {
    ...
    String dest(String tool, [String session = 'sess-1']) =>
        p.join(configDir(session), 'plugins', 'marketplaces', 'demo');
    ...
    void createClone(String tool, String session, {bool stamped = false}) {
      ...
      File(
        p.join(d, '.claude-plugin', 'marketplace.json'),
      ).createSync(recursive: true);
      if (stamped) {
        File(
          p.join(d, '.teampilot-marketplace-source-stamp.json'),
        ).writeAsStringSync('{}');
      }
    }
    ...
    test('lstat distinguishes a symlink from its directory target', () async {
      ...
      expect(
        (await fs.stat(d)).isDirectory,
        isTrue,
        reason: 'stat follows the link to the target',
      );
      expect((await fs.lstat(teampilotRoot)).isDirectory, isTrue);
      expect(
        (await fs.lstat(p.join(teampilotRoot, 'missing'))).exists,
        isFalse,
      );
    });

    test(
      'ensureSessionLinked symlinks to the shared flavor dir, not the git cache',
      () async {
        createCache();
        ...
        expect(
          File(
            p.join(flavor, '.claude-plugin', 'marketplace.json'),
          ).existsSync(),
          isTrue,
        );
    ...
    test('non-neutral flavor dir carries the flavor projection', () async {
      ...
      expect(
        File(
          p.join(flavor, '.flashskyai-plugin', 'marketplace.json'),
        ).existsSync(),
        isTrue,
    ...
    test(
      'ensureShared is idempotent and refreshes when the cache changes',
      () async {
        ...
        if (Platform.isWindows) {
          markTestSkipped(
            'Windows copyTree re-materialization yields empty file',
          );
          return;
        }
        createCache();
    ...
    test(
      'ensureSessionLinked is idempotent and leaves real dirs untouched',
      () async {
        createCache();
    ...
    test(
      'ensureSessionLinked falls back to copy when symlinks are unavailable',
      () async {
        final memFs = _NoSymlinkFilesystem();
        final memStore = MarketplaceSharedStore(
          fs: memFs,
          teampilotRoot: '/tp',
        );
        final memPath = memFs.pathContext;
        final memCache = memPath.join(
          '/tp',
          'plugins',
          'marketplace-cache',
          'owner',
        ...
        expect(
          await memFs.readString(
            memPath.join(memDest, '.claude-plugin', 'marketplace.json'),
          ),
          '{}',
        );
      },
    ...
    test(
      'ensureSessionMarketplacesLinked links known marketplaces from config',
      () async {
        File(
          p.join(teampilotRoot, 'plugins', 'marketplaces.json'),
        ).createSync(recursive: true);
        await File(
          p.join(teampilotRoot, 'plugins', 'marketplaces.json'),
        ).writeAsString(
    ...
    group('sweepAll', () {
      ...
      test(
        'replaces unstamped clones, keeps stamped and active sessions',
        () async {
          createCache();
      ...
      test('covers mixed member-nested config dirs', () async {
        ...
        File(
          p.join(memberDest, '.claude-plugin', 'marketplace.json'),
        ).createSync(recursive: true);
