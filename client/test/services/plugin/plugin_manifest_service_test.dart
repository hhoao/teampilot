...
void main() {
  ...
  test('parses plugin.json with version and description', () async {
    ...
    File(p.join(dir.path, '.claude-plugin', 'plugin.json')).writeAsStringSync(
      '{"name":"my-plugin","version":"1.2.3","description":"hi"}',
    );
  ...
  test('falls back to directory name when plugin.json missing', () async {
    ...
    File(p.join(dir.path, 'commands', 'deploy.md')).writeAsStringSync(
    ...
    expect(
      result.capabilities.commands.first.description,
  ...
  test('parses hooks.json and .mcp.json', () async {
    ...
    File(
      p.join(dir.path, 'hooks', 'hooks.json'),
    ).writeAsStringSync('{"hooks":{"PreCommit":[{"matcher":"*.dart"}]}}');
