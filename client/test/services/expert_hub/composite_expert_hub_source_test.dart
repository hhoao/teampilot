...
void main() {
  test('fetchMembers includes builtin templates', () async {
    ...
    expect(members.any((m) => m.key == 'teampilot/builtin/developer'), isTrue);
    expect(
  ...
  test(
    'team roster with builtin expert keys does not duplicate catalog',
    () async {
      ...
      final team = DiscoverableTeam(
        ...
        cli: CliTool.claude,
        roster: [
          TeamRosterSlot(id: 'developer', expertKey: developerBuiltin.key),
        ],
      );
  ...
  test(
    'team roster with custom expert keys is indexed for discovery',
    () async {
      ...
      final team = DiscoverableTeam(
        ...
        updatedAt: 1,
        cli: CliTool.claude,
        roster: const [TeamRosterSlot(id: 'researcher', expertKey: customKey)],
      );
