...
ExtensionDetector _present(String version) => ExtensionDetector(
  processRunner: (exe, args, {environment}) async {
    ...
    if (args.contains('--version')) {
      return ProcessResult(0, 0, 'x $version', '');
    }
...
void main() {
  ...
  test('cargo runs cargo install', () async {
    ...
    await engine.install(
      _manifest({'kind': 'cargo', 'package': 'rtk', 'binary': 'rtk'}),
    );
    ...
    final engine = ExtensionAcquisitionEngine(
      runner: (cmd) async {
        ...
        return CliInstallerCommandResult(
          exitCode: cmd.executable == 'cargo' ? 1 : 0,
        );
      },
      detector: _present('0.24.0'),
    );
  ...
  test('fails when all commands fail', () async {
    final engine = ExtensionAcquisitionEngine(
      runner: (cmd) async =>
          const CliInstallerCommandResult(exitCode: 1, stderr: 'nope'),
      detector: _present('1.0.0'),
    );
    final result = await engine.install(
      _manifest({'kind': 'cargo', 'package': 'rtk'}),
    );
  ...
  test('script kind accepts an https url', () async {
    ...
    expect(
      commands.single.arguments.last,
      contains('https://example.com/install.sh'),
    );
  });

  test(
    'script kind rejects non-https / metacharacter urls (no command run)',
    () async {
      final commands = <CliInstallerCommand>[];
      final engine = ExtensionAcquisitionEngine(
        runner: (cmd) async {
