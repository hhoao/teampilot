...
void main() {
  test('merges matches from all slices, each tagged with its slice', () async {
    ...
    final events = await search
        .run(const TpSearchOptions(pattern: 'h'))
        .toList();
    final roots = events.map((e) => e.slice.root).toSet();
    expect(roots, {'/a', '/b'});
    expect(
      events.where((e) => !e.isError).map((e) => e.match!.path),
      containsAll(['/a/x.dart', '/b/y.dart']),
    );
  });

  test(
    'a failing slice emits one error event; other slices keep streaming',
    () async {
      final a = _FakeRunner('/a');
      final b = _FakeRunner('/b');
