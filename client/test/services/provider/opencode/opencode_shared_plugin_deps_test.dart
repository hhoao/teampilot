...
void main() {
  ...
  test('shared root is workspace/cache not cli-defaults', () {
    final seeder = OpencodeSharedPluginDeps(layout: layout, fs: fs);
    expect(
      seeder.sharedRoot,
      p.join(base.path, 'workspace', 'cache', 'cli', 'opencode', '_shared'),
    );
  });

  test(
    'seed writes package.json and installs when plugin dir missing',
    () async {
      var installCalls = 0;
      final cacheRoot = WorkspaceCliCache(
        layout: layout,
      ).globalRoot(tool: 'opencode');
      ...
      final pkg = await File(
        p.join(seeder.sharedRoot, 'package.json'),
      ).readAsString();
      expect(pkg, contains('"@opencode-ai/plugin": "1.18.4"'));
      expect(
        await Directory(
          p.join(seeder.sharedRoot, 'node_modules', '@opencode-ai', 'plugin'),
        ).exists(),
        isTrue,
      );
    },
  ...
  test('second ensure skips npm install when complete', () async {
    ...
    var installCalls = 0;
    final skipper = OpencodeSharedPluginDeps(
      layout: layout,
      fs: fs,
      resolvePluginVersion: () async => '1.18.4',
      npmInstall: (_) async {
    ...
    await skipper.ensureSharedInstalled();
  ...
  test('failed install removes incomplete node_modules', () async {
    final seeder = OpencodeSharedPluginDeps(
      ...
      npmInstall: (cwd) async {
        await Directory(
          p.join(cwd, 'node_modules', 'partial'),
        ).create(recursive: true);
        return 1;
      },
    );
    ...
    expect(
      await Directory(p.join(seeder.sharedRoot, 'node_modules')).exists(),
      isFalse,
    );
  });
  ...
  test('migrates complete cli-defaults node_modules into cache', () async {
    final legacy = layout.appToolRoot('opencode');
    await Directory(
      p.join(legacy, 'node_modules', '@opencode-ai', 'plugin'),
    ).create(recursive: true);
    await File(p.join(legacy, 'package.json')).writeAsString('{"ok":true}');
    ...
    expect(
      await File(p.join(seeder.sharedRoot, 'package.json')).readAsString(),
      '{"ok":true}',
    );
    expect(await Directory(p.join(legacy, 'node_modules')).exists(), isFalse);
  });
}
