...
static void zikzak_inappwebview_linux_plugin_handle_method_call(
  ...
  if (strcmp(method, "getPlatformVersion") == 0) {
    ...
    // TeamPilot patch: upstream has no webview teardown path — the
    // web_views table only grows, so every create leaks a permanent
    // offscreen WebKitGTK instance with a 33ms render loop. Dropping the
    // entry unrefs the webview; the remaining refs (texture registrar,
    // per-webview channel) release it once the Flutter widget tree tears
    // down, which disposes the offscreen window and stops the timer.
    if (fl_value_get_type(args) == FL_VALUE_TYPE_MAP) {
      FlValue *idVal = fl_value_lookup_string(args, "id");
      if (idVal && fl_value_get_type(idVal) == FL_VALUE_TYPE_STRING) {
        g_hash_table_remove(self->web_views, fl_value_get_string(idVal));
      }
    }
    response = FL_METHOD_RESPONSE(
