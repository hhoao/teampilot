...
TeamPilot fork of 0.5.2. See [TEAMPILOT_PATCHES.md](TEAMPILOT_PATCHES.md).

* [windows] Mark `NonRudeHWND` for custom chrome so auto-hide taskbar still pops when maximized (#438)
* [windows] Skip USER client BitBlt via `WVR_VALIDRECTS` (empty rects) to avoid resize ghost borders without full-window redraw lag
* [windows] Cheap `InvalidateRect` after live resize ends
