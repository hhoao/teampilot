...
std::optional<LRESULT> WindowManagerPlugin::HandleWindowProc(HWND hWnd,
  ...
  if (wParam && message == WM_NCCALCSIZE) {
    // Returning 0 lets USER BitBlt the old client into the new client
    // (top-left aligned). Custom chrome shrinks the client by ~8px for the
    // resize border, so that blit leaves ghost borders. Returning WVR_REDRAW
    // also stops the blit but forces a full-window invalidate every size
    // tick - Flutter cannot keep up, so resize feels slow and ghosts linger.
    // WVR_VALIDRECTS with empty source/dest skips the blit without that cost.
    auto skip_client_blit = [](NCCALCSIZE_PARAMS* sz) -> LRESULT {
      SetRectEmpty(&sz->rgrc[1]);
      SetRectEmpty(&sz->rgrc[2]);
    ...
    if (window_manager->is_resizing_) {
      ...
      // One cheap invalidate after live resize - avoid ForceChildRefresh's
      // +1px SetWindowPos dance (visible hitch on large Flutter surfaces).
