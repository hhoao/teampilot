...
class WindowManager {
 public:
  ...
  // Marks/unmarks the HWND so Shell does not treat a custom-chrome maximized
  // window as fullscreen (auto-hide taskbar can still pop). See NonRudeHWND
  // remarks on ITaskbarList2::MarkFullscreenWindow.
  void WindowManager::SetNonRudeHwnd(bool enabled);
...
void WindowManager::SetAsFrameless() {
  ...
  SetWindowPos(hWnd, nullptr, rect.left, rect.top, rect.right - rect.left,
               ...
               SWP_NOZORDER | SWP_NOOWNERZORDER | SWP_NOMOVE | SWP_NOSIZE |
                   SWP_FRAMECHANGED);
  // Frameless maximize covers the monitor edge the same way TitleBarStyle.hidden
  // does; keep the Shell from treating it as a rude fullscreen window.
  // Callers that enter true fullscreen must clear this afterward.
  SetNonRudeHwnd(true);
}
...
void WindowManager::Show() {
  HWND hWnd = GetMainWindow();
  // Must be set before the window is shown so Shell does not treat a
  // custom-chrome window as fullscreen when maximized (auto-hide taskbar).
  // https://github.com/leanflutter/window_manager/issues/438
  if (title_bar_style_ == "hidden" || is_frameless_) {
    SetNonRudeHwnd(true);
  }
...
void WindowManager::SetFullScreen(const flutter::EncodableMap& args) {
  ...
  if (isFullScreen) {  // Set to fullscreen
    ...
    // After SetAsFrameless (which marks NonRudeHWND), clear it so true
    // fullscreen keeps the auto-hide taskbar suppressed.
    SetNonRudeHwnd(false);
  } else {  // Restore from fullscreen
    // if (!g_maximized_before_fullscreen)
    //   Restore();
    if (g_title_bar_style_before_fullscreen == "hidden" || is_frameless_) {
      SetNonRudeHwnd(true);
    }
    ::SetWindowLongPtr(
        mainWindow, GWL_STYLE,
...
void WindowManager::SetTitleBarStyle(const flutter::EncodableMap& args) {
  ...
  SetWindowPos(hWnd, nullptr, rect.left, rect.top, 0, 0,
               SWP_NOZORDER | SWP_NOOWNERZORDER | SWP_NOMOVE | SWP_NOSIZE |
                   SWP_FRAMECHANGED);
  // TeamPilot (and other custom-chrome apps) often call this after the window
  // is already visible; still mark NonRudeHWND for maximize + auto-hide taskbar.
  SetNonRudeHwnd(title_bar_style_ == "hidden");
}
