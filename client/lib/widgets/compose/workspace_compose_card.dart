...
class WorkspaceComposeCard extends StatelessWidget {
  ...
  List<Widget> _launchErrorBanner(
    ...
    final error = chrome.launchError?.trim();
    if (error == null || error.isEmpty) return const [];
    final deadTargetId = deadSshTargetIdFromError(chrome.launchError);
    final remapHandler = chrome.onRemapDeadTarget;
    final showRemap = deadTargetId != null && remapHandler != null;
