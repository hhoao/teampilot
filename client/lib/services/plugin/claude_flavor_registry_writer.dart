...
class ClaudeFlavorRegistryWriter {
    ...
    final scanned = await Future.wait(
      entries.map((entry) async {
        ...
        final marketplaceKey =
            catalogPlugin?.marketplaceName ?? localMarketplaceName;
        ...
        final localMarketplacePlugin = marketplaceKey == localMarketplaceName
            ? {
                'name': pluginName,
                'source': './$pluginName',
  ...
  Future<String> _resolveCliPluginName({
    ...
    if (marketplaceKey == localMarketplaceName) {
  ...
  Future<List<Map<String, Object?>>> _writeKnownMarketplaces({
    ...
    await fs.atomicWrite(
      fs.pathContext.join(pluginsDir, knownMarketplacesFileName),
  ...
  Future<String?> _materializeMarketplaceForCli({
    ...
    if (await CliPluginProvisionCache.isMarketplaceMaterializationCurrent(
      ...
      appLogger.d(
        '[CliPluginRegistry] materializeMarketplace: skipped copy ($marketplaceName)',
      );
      return dest;
    }

    var linked = await CliPluginLayout.linkOrCopyTree(
      fs: fs,
      source: teampilotCacheDir,
      destination: dest,
    );
    // Flavor projection needs a materialized tree (symlink targets skip sibling writes).
    if (linked) {
  ...
  Future<String> _writeLocalMarketplaceStub({
    ...
    if (!(await fs.stat(manifestPath)).isFile) {
      ...
      await CliPluginLayout.projectBundleToFlavor(fs, stubRoot, paths);
    }
    return stubRoot;
  }
