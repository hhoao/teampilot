...
abstract final class UiInteractiveWarmup {
  ...
  static Future<void> _warmGlyphs({
    ...
    // Style fingerprints only — short [TpGlyphWarmup.styleProbe], not l10n dumps.
    TpGlyphWarmup.shapeAll(styles: styles);
    // Nested markdown spans (bold/mono etc.) — single-style shapeAll is not enough.
    warmMarkdownMixedInlineLayout(theme);
    appLogger.i(
      '[boot] glyph warmup shape done +${sw.elapsedMilliseconds}ms '
      '(+ mixed inline paths)',
    );
  }
