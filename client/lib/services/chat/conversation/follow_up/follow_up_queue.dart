...
final class FollowUpQueue {
  ...
  }) => FollowUpQueue(items: items ?? this.items, drain: drain ?? this.drain);
}
...
final class InMemoryFollowUpQueueStore {
  ...
  FollowUpQueue queueFor(String seat) => _queues[seat] ?? const FollowUpQueue();
  ...
  void enqueue(String seat, String content) {
    ...
    final drain = q.items.isEmpty ? FollowUpDrainMode.armed : q.drain;
    _emit(
      seat,
      q.copyWith(
        items: [
      ...
      q.copyWith(
        items: [
          for (final m in q.items)
            if (m.id == id)
              FollowUpQueuedMessage(id: id, content: trimmed)
            else
              m,
        ],
      ),
