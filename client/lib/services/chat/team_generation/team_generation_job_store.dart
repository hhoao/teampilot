...
final class TeamGenerationJobStore {
  ...
  WorkspaceLayout get _layout =>
      _layoutOverride ??
      WorkspaceLayout(teampilotRoot: _storage.paths.basePath, fs: _storage.fs);
  ...
  Future<TeamGenerationJob?> read(String workspaceId, String workflowId) async {
    ...
    try {
      ...
      appLogger.w(
        '[team-generation] unreadable job $workspaceId/$workflowId: $e',
      );
      return null;
    }
  }
  ...
  bool _canReplaceReceipt(
    TeamGenerationReceipt? before,
    TeamGenerationReceipt after,
  ) =>
      before == null ||
  ...
  Future<TeamGenerationJob> resumeFailed(
    String workspaceId,
    String workflowId,
  ) {
    return _synchronized(workspaceId, workflowId, () async {
  ...
  Future<void> _pruneTombstones(String workspaceId) async {
    ...
    final cutoff = _clock()
        .subtract(teamGenerationTombstoneMaxAge)
    ...
    final remaining =
        (await listAll(
            workspaceId,
          )).where((job) => job.phase == TeamGenerationPhase.complete).toList()
          ..sort((a, b) => b.updatedAt.compareTo(a.updatedAt));
